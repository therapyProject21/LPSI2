...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Query untuk insert data (tanpa Token_Verifikasi)
    $sql = "INSERT INTO Pengguna (Nama, Alamat, Nomor, Password_LPSI)
            VALUES (?, ?, ?, ?)";

    // Gunakan prepared statement untuk menghindari SQL injection
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssss", $nama, $alamat, $nomor, $password_lpsi);
    ...
    if ($stmt->execute()) {
        ...
        echo "<div class='bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mb-4' role='alert'>
                <strong>Error!</strong> Terjadi kesalahan: " . $stmt->error . "
              </div>";
    }
